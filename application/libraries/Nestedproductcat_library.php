...
class Nestedproductcat_library {
    ...
    function cmbNested($data, $action = '', $id_edit = '', $row = '') {
        ...
        if ( sizeof($data) > 0 ) {
            ...
            if ( $cat_name != '' ) {
                $arr['product_cat_id'] = $product_cat_id;
    ...
    function formatTree($tree, $parent) {
        ...
        foreach ($tree as $i => $item) {
            if ($item['parents'] == $parent) {
                $tree2[$item['product_cat_id']] = $item;
                $tree2[$item['product_cat_id']]['submenu'] = $this->formatTree($tree, $item['product_cat_id']);
            }
        }
    ...
    function catStringID($cats) {
        ...
        foreach ($cats as $vl) {
            $strId .= ($i > 1) ? ',' . $vl['product_cat_id'] : $vl['product_cat_id'];
     ...
     * @return string
     */
    function cmbproduct_cat_idLanding($data, $action = '', $id_edit, $row = '') {
        $cat_name = '';
        $cmb = '';
        $arr = array();
    ...
    function groupSub($data) {
        ...
        foreach ($data as $key => $val){
            if($val['level'] == 1) {
                $val['sub'] = $this->formatTree($data, $val['product_cat_id']);
                $temp[] = $val;
            }
        }
