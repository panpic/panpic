...
class Slim_library_banner {
    ...
    public static function getImages($inputName = 'slim') {
        ...
        $values = Slim_library_banner::getPostData($inputName);
        ...
        foreach ($values as $value) {
            $inputValue = Slim_library_banner::parseInput($value);
            if ($inputValue) {
    ...
    private static function parseInput($value) {
        ...
        if (isset ($data->input)) {
            ...
            if (isset($data->input->image)) {
                $inputData = Slim_library_banner::getBase64Data($data->input->image);
            }
        ...
        if (isset($data->output)) {
            ...
            if (isset($data->output->image)) {
                $outputData = Slim_library_banner::getBase64Data($data->output->image);
            }
    ...
    public static function saveFile($data, $name, $path = 'tmp/', $uid = true) {
        ...
        // $name = Slim_library_banner::sanitizeFileName($name);
        ...
        // store the file
        Slim_library_banner::save($data, $path);
