...
class Album extends MY_Controller{
    ...
    public function __construct(){
        ...
        $this->_post_type = POST_TYPE_ALBUM;
        $this->_blog_parent_category = PARENT_CAT_ALBUM;
        $this->_post_type_gallery_detail = POST_TYPE_ALBUM_DETAIL;
    ...
    function index(){
        ...
        $this->_data['breadcrumb'] = $this->lable['mn_album_gallery'];
        $this->_data['alert'] = '';
    ...
    function duplicate() {
        ...
        $id = $this->input->get('id');
        if (!$id) {
            redirect(admin_url('album/items'));
        }
    ...
    function items($start=0){
        ...
        $base_url    = admin_url('album/items');
        $uri_segment = 4;
    ...
    function gallery() {
        error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
        $post_type = POST_TYPE_BLOG_GALLERY;
        ...
        if ($this->input->post()) {
            ...
            if (!empty($primary) && $option == OPTION_EDIT) { //Update
                ...
                if ($update) {
                    ...
                    $this->session->set_flashdata('alert', 'success');
                    $this->session->set_flashdata('msg', $this->lable['edit_succ']);
                    redirect( admin_url("album/gallery?album=$album_id") );
                } else {
                    $this->session->set_flashdata('alert', 'danger');
                    $this->session->set_flashdata('msg', $this->lable['edit_fail']);
                    redirect( admin_url("album/gallery?album=$album_id&id=$primary&option=$option") );
                }
                ...
                if ($insert) {
                    ...
                    $this->_data['msg'] = $this->lable['add_fail'];
                    $this->_data['data'] = $_data;
                    $this->parser->parse("album/gallery.tpl", $this->_data);
                    return;
                }
            }
        }
    ...
    /**
     * Update image gallery detail
     *
     * @param int $id
     * @param int $avail
     */
    function gallery_avail() {
        error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
        ...
        if($id != '') {
            ...
            echo $this->gallery_model->updateByFields(array('avail'=>$avail));
            return;
        }
    ...
    function do_slim_upload_gallery($blog_id) {
        ...
        if($new_img){
            ...
            $params = array(
                'object_id'     => $blog_id,
                'image_type'    => $this->_post_type_gallery_detail,
                'path_image'    => $path_image,
            );
            ...
            if($old_img != '' && $old_file_name != $filename) {
                unlink( $this->_path_upload.'/'.$old_img );
                $imageOldExist = $this->blogs_model->getImageByBlogId($blog_id, $cond=" AND image_type = '$this->_post_type_gallery_detail' ");
                if($imageOldExist != '') {
                    unlink( $this->_path_upload.'/'.$imageOldExist['path_image_thumb'] );
                    $image_id = $old['image_id'];
					$params['path_image_thumb'] = ($params['path_image_thumb'] == '') ? '' : $params['path_image_thumb'];
