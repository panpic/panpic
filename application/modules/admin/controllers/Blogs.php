...
class Blogs extends MY_Controller{
    ...
    function index(){
        ...
        if (!empty($id) && $option == 'edit') {
            ...
            $data = $this->blogs_model->getItemsMultiLangById($cond_image); // getItemById($cond_image);
            $first = $data[0];
            ...
            $this->blogs_model->id = $id;
            $tags = $this->blogs_model->getProductTagsByProductId();
        } else {
        ...
        $cond = " AND c.post_cat_id <> $this->_blog_parent_category ";
        ...
        if (!empty($id) && $option == 'edit') {
            ...
            $this->parser->parse("blogs/add.tpl", $this->_data);
        }

        // $img = $this->_path_upload.'2018/08/27/180827-064436-001-36.jpg';
    ...
    function add() {
        ...
        $_tags_value = $this->input->post('tags');
        $_tags_value = isset($_tags_value[0]) ? $_tags_value[0] : '';
        ...
        if( $this->form_validation->run() == FALSE ) {
            ...
            $this->parser->parse("blogs/$tpl", $this->_data);
            return;
        }
        ...
        if (!empty($id) && $option == OPTION_EDIT) { //Update
            ...
            */
            ...
            $this->blogs_model->id = $id;
            $update = $this->blogs_model->updateItem($_data, $params);
            ...
            if ($update) {
                ...
                redirect(admin_url("blogs/?id=$id&option=edit"));
            }
        } else { //insert
    ...
    function duplicate() {
        error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
        $id = $this->input->get('id');
        $back_url = admin_url("blogs/items");
        ...
        $_data_zh = $this->blogs_model->getItemByCond($cond_zh, $cond_img);
        $slug_zh = $this->blogs_model->get_slug_exist($_data_zh['slug']."-".DUPLICATED);
        $params_zh = array(
            'lang'          => $_data_zh['lang'],
            'slug'          => $slug_zh,
        ...
        $insert = $this->blogs_model->duplicateItem($params, $params_vi, $params_en, $params_zh);
    ...
    */
    function items($start=0){
        ...
        */
        ...
        $this->parser->parse("blogs/items.tpl", $this->_data);
    }
    ...
    function restore() {
        ...
        if($blog_id && $blog_id > 0){
            ...
            $this->blogs_model->updateBlogByFields(array('avail' => 1));
        }
    ...
    function show_home() {
        ...
        $display_home = $this->input->post('d');
    ...
    function ajx_tags() {
        ...
        // $q = mb_strtolower(trim($txtKey), 'UTF-8');
        // $q = trim($txtKey);

        $rs = $this->blogs_model->searchTags($q);

        if(sizeof($rs) > 0) {
            foreach ($rs as $vl) {
                $temp[] = array('id' => $vl['title'], 'text' => $vl['title'], 'value' => $vl['title']);
            }
        } else {
            $temp[] = array('id' => $q,'text'=> $q, 'value'=> $q);
        }
