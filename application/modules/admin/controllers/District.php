...
class District extends MY_Controller{
	...
	public function __construct(){
        ...
        $this->district_model->current_lang = $this->current_lang;
	...
	function index()
	{
        ...
        $more_url = '';
		$this->_data['breadcrumb'] = $this->lable['district'];
        ...
        $totalItems  = $this->district_model->countItems($cond);
        ...
        $items = $this->district_model->getItems($cond, $per_page, $start);
        ...
        if( $this->input->post()) { // Delete to Recyle bin
            ...
            if( sizeof($arr) > 0 ) {
                ...
                $params['avail'] = 0;
                foreach ($arr as $vl) {
                    $this->district_model->id = $vl;
                    $update = $this->district_model->updateItem($params);
                }
	...
	function add(){
        ...
        if (!empty($id) && $option == 'edit') {
            ...
            $data = $this->district_model->getItemById();
        }
        ...
        if( $this->input->post() ) {
            ...
            $params = $this->district_model->buildDataInsert($_data);
            ...
            if (!empty($id) && $option == OPTION_EDIT) { //Update
                ...
                $insert = $this->district_model->insertItem($params);
                if ($insert) {
                    $this->session->set_flashdata('alert', 'success');
    ...
    function removemulti() {
        ...
        if(sizeof($checkAll) > 0) {
            ...
            foreach ($checkAll as $id) {
                $this->district_model->deleteItem( array('district_id' => $id) );
            }
	...
	function del() {
        $mess['not_error'] = '0';
        $id = (int) $this->input->get('id');
        // $this->district_model->deleteItem( array('state_id' => $id) );
    }
	...
	public function ajaxState(){
		...
		if($this->input->post()) {
			...
			$this->district_model->data_filter = $data_filter;
			$dataPro = $this->district_model->getItems();
