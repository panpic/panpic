...
class Download extends MY_Controller{
    ...
    public function __construct(){
        ...
        $this->_post_type = POST_TYPE_DOWNLOAD;
        $this->_blog_parent_category = POST_CAT_DOWNLOAD;
        ...
        $this->download_model->current_lang = $this->current_lang;
        $this->download_model->page_lang = $this->page_lang;
        $this->download_model->default_lang = $this->default_lang;
        ...
        $this->_allow_upload_file = $this->config->item("allowed_file_types_profile");
        $this->_data['allow_upload_file'] = $this->_allow_upload_file;
    ...
    function index(){
        ...
        $this->_data['breadcrumb'] = $this->lable['download'];
        $this->_data['alert'] = '';
        ...
        if (!empty($id) && $option == 'edit') {
            $this->download_model->id = $id;
            $cond_image=" AND c.image_type = '$this->_post_type' ";
            $data = $this->download_model->getItemsMultiLangById($cond_image); // getItemById($cond_image);
            $first = $data[0];
    ...
    function add() {
        ...
        if($file['file_name'] != '') {
            if($file['error'] != '') {
                ...
                if($old_file != '') {
                    unlink($this->_path_upload.'/'.$old_file );
                }
            }
        }

        if($this->form_validation->run() == FALSE || $error_file != '') {
            ...
            $params = array(
                ...
                'short'     => trim($_data[$this->current_lang]['short']),
                'title_2'   => $path_file,
                'date_add'  => $_data['date_add'],
                'seo_title'     => trim($_data[$this->current_lang]['seo_title']),
                'seo_description' => trim($_data[$this->current_lang]['seo_description']),
            );
        ...
        if (!empty($id) && $option == OPTION_EDIT) { //Update
            ...
            if($path_file != '') {
                $_data['path_file'] = $path_file;
            }

            $this->download_model->id = $id;
            $update = $this->download_model->updateItem($_data, $params);
            ...
            $_data['path_file']   = $path_file;
            $insert = $this->download_model->insertItem($params, $_data);
    ...
    function duplicate() {
        ...
        $id = $this->input->get('id');
        if (!$id) {
            redirect(admin_url('download/items'));
        }
        ...
        $_data_vi = $this->download_model->getItemByCond($cond_vi, $cond_img);
        $slug_vi = $this->download_model->get_slug_exist($_data_vi['slug']."-".DUPLICATED);

        $cond_en = " AND a.blog_id = $id  AND b.lang = '".LANG_EN."' ";
        $_data_en = $this->download_model->getItemByCond($cond_en, $cond_img);
        $slug_en = $this->download_model->get_slug_exist($_data_en['slug']."-".DUPLICATED);
        ...
        $insert = $this->download_model->duplicateItem($params, $params_vi, $params_en);

        if ($insert) {
            $image_blog = $this->download_model->getImageByBlogId($id, " AND image_type = '$this->_post_type' ");
    ...
    function items($start=0){
        ...
        $totalItems  = $this->download_model->countItems($cond_total);
        $per_page    = $this->lable['per_item_admin'];
        $base_url    = admin_url('download/items');
        $uri_segment = 4;
        ...
        $items = $this->download_model->getItems($cond_items, $per_page, $start);
        $this->_data['items'] = $items;
    ...
    function deletemulti() {
        ...
        foreach ($checkAll as $id) {
            $this->download_model->id = $id;
            $this->download_model->updateBlogByFields( array('avail' => 0) );
        }
    ...
    function removemulti() {
        ...
        foreach ($checkAll as $id) {
            $this->download_model->id = $id;
            $data = $this->download_model->getItemById(" AND c.image_type = '$this->_post_type' ");
            @unlink($this->_path_upload.$data['title_2']);
            $this->download_model->deleteItem( array('blog_id'=>$id) );
        }

        $back_url = admin_url($this->_control."/items/?t=5");
        $this->session->set_flashdata('alert', 'success');
        $this->session->set_flashdata('msg', $this->lable['delete_succ']);
        redirect( $back_url );
    ...
    function restore() {
        ...
        if($blog_id && $blog_id > 0){
            $this->download_model->id = $blog_id;
            $this->download_model->updateBlogByFields(array('avail' => 1));
        }
    ...
    function update_status() {
        ...
        if($blog_id != '') {
            $this->download_model->id = $blog_id;
            echo $this->download_model->updateBlogByFields(array('avail'=>$avail));
            return;
        }
    ...
    function updateVerify() {
        ...
        if($blog_id != '') {
            $this->download_model->id = $blog_id;
            echo $this->download_model->updateBlogByFields( array('admin_verify' => $display_home) );
            return;
        }
    ...
    function show_home() {
        ...
        if($blog_id != '') {
            $this->download_model->id = $blog_id;
            $this->download_model->current_lang = $lang;
            echo $this->download_model->updateBlogTranslateByFields( array('home_status' => $display_home) );
            return;
        }
    ...
    public function do_upload()
    {
        ...
        $config['upload_path']      = $file_path_day;
        $config['allowed_types']    = $this->_allow_upload_file;
        $config['max_size']         = 1024 * 300; // <= 200Mb;
        ...
        {
            $data = $this->upload->data();
            $data['path_file'] = $new_path.$data['file_name'];
            return $data;
        }
    }
    ...
    function downloadfile() {
        ...
        $file_name = $this->_path_upload.$data['title_2'];
