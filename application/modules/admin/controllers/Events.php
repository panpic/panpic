...
class Events extends MY_Controller{
    ...
    public function __construct(){
        ...
        $this->_post_type = POST_TYPE_EVENT;
        $this->_blog_parent_category = PARENT_CAT_EVENT;
    ...
    function index(){
        ...
        if (!empty($id) && $option == 'edit') {
            $this->parser->parse("events/edit.tpl", $this->_data);
        } else {
            $this->parser->parse("events/add.tpl", $this->_data);
        }
    ...
    function add() {
        ...
        if($this->form_validation->run() == FALSE) {
            ...
            $this->parser->parse("events/$tpl", $this->_data);
            return;
        }
        ...
        if (!empty($id) && $option == OPTION_EDIT) { //Update
            ...
            $params['date_add'] = $_data['date_add'];
            ...
            if ($update) {
                ...
                redirect( admin_url("events/items/") );
            } else {
                $this->session->set_flashdata('alert', 'danger');
                $this->session->set_flashdata('msg', $this->lable['edit_fail']);
                redirect( admin_url("events/?id=$id&option=edit") );
            }
        } else { //insert
    ...
    function duplicate() {
        ...
        if ($insert) {
            ...
            $this->parser->parse("events/alert.tpl", $this->_data);
    ...
    function items($start=0){
        ...
        */
        ...
        $base_url    = admin_url('events/items');
        $uri_segment = 4;
        ...
        $this->parser->parse("events/items.tpl", $this->_data);
    }
