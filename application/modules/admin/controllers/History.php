...
class History extends MY_Controller{
    ...
    public function __construct(){
        ...
        $this->_post_type = POST_TYPE_HISTORY;
        $this->_blog_parent_category = 0;
        $this->_cat_portoflio_pos = 0;
        ...
        $this->history_model->current_lang = $this->current_lang;
        $this->history_model->page_lang = $this->page_lang;
        $this->history_model->default_lang = $this->default_lang;
    ...
    function index(){
        ...
        $this->_data['breadcrumb'] = $this->lable['history'];
        $this->_data['alert'] = '';
        ...
        if (!empty($id) && $option == 'edit') {
            $this->history_model->id = $id;
            $cond_image=" AND c.image_type = '$this->_post_type' ";
            $data = $this->history_model->getItemsMultiLangById($cond_image);
        ...
        */
        ...
        if (!empty($id) && $option == OPTION_EDIT) { // Update
            ...
            $params_translate = array(
                ...
                'short'     => $_data[$this->current_lang]['short'],
                'content'   => addslashes($_data[$this->current_lang]['content']),
            ); 

            $slug = url_convert($_data[$this->current_lang]['slug']);
            $slug = $this->history_model->get_slug_exist($slug, $id);
            $params_translate['slug'] = $slug;
            $this->history_model->current_lang = $_data['lang'];
            */

            $this->history_model->id = $id;
            $update = $this->history_model->updateItem($_data, $params);
            ...
            $insert = $this->history_model->insertItem($params, $_data);
    ...
    function duplicate() {
        ...
        $_data_vi = $this->history_model->getItemByCond($cond_vi, $cond_img);
        $slug_vi = $this->history_model->get_slug_exist($_data_vi['slug']."-".DUPLICATED);

        $cond_en = " AND a.blog_id = $id  AND b.lang = '".LANG_EN."' ";
        $_data_en = $this->history_model->getItemByCond($cond_en, $cond_img);
        $slug_en = $this->history_model->get_slug_exist($_data_en['slug']."-".DUPLICATED);

        $cond_zh = " AND a.blog_id = $id  AND b.lang = '".LANG_ZH."' ";
        $_data_zh = $this->history_model->getItemByCond($cond_zh, $cond_img);
        $slug_zh = $this->history_model->get_slug_exist($_data_zh['slug']."-".DUPLICATED);
        // pre($_data_zh);

        $params = array(
            'category_id'   => $_data_vi['category_id'],
            'portfolio_category_id' => $_data_vi['portfolio_category_id'],
            'post_type'     => $this->_post_type,
        ...
        $params_vi = array(
            ...
            'portfolio_services'    => $_data_vi['portfolio_services'],
            'portfolio_utility'     => $_data_vi['portfolio_utility'],
            'title_2'               => $_data_vi['title_2'],
            'short'         => $_data_vi['short'],
            'content'       => $_data_vi['content'],
            'content_2'     => $_data_vi['content_2'],
            'price'         => $_data_vi['price'],
            'portfolio_ntc' => $_data_vi['portfolio_ntc'],
            'portfolio_ntdul'=> $_data_vi['portfolio_ntdul'],
            'portfolio_qm'   => $_data_vi['portfolio_qm'],
            'portfolio_dt'   => $_data_vi['portfolio_dt'],
            'portfolio_st'   => $_data_vi['portfolio_st'],
        ...
        $params_en = array(
            ...
            'portfolio_services'    => $_data_en['portfolio_services'],
            'portfolio_utility'     => $_data_en['portfolio_utility'],
            'title_2'               => $_data_en['title_2'],
            'short'         => $_data_en['short'],
            'content'       => $_data_en['content'],
            'content_2'     => $_data_en['content_2'],
            'price'         => $_data_en['price'],
            'portfolio_ntc' => $_data_en['portfolio_ntc'],
            'portfolio_ntdul'=> $_data_en['portfolio_ntdul'],
            'portfolio_qm'   => $_data_en['portfolio_qm'],
            'portfolio_dt'   => $_data_en['portfolio_dt'],
            'portfolio_st'   => $_data_en['portfolio_st'],
        ...
        $params_zh = array(
            'lang'          => $_data_zh['lang'],
            'slug'          => $slug_zh,
            'title'         => $_data_zh['title'],
            'portfolio_services'    => $_data_zh['portfolio_services'],
            'portfolio_utility'     => $_data_zh['portfolio_utility'],
            'title_2'               => $_data_zh['title_2'],
            'short'         => $_data_zh['short'],
            'content'       => $_data_zh['content'],
            'content_2'     => $_data_zh['content_2'],
            'price'         => $_data_zh['price'],
            'portfolio_ntc' => $_data_zh['portfolio_ntc'],
            'portfolio_ntdul'=> $_data_zh['portfolio_ntdul'],
            'portfolio_qm'   => $_data_zh['portfolio_qm'],
            'portfolio_dt'   => $_data_zh['portfolio_dt'],
            'portfolio_st'   => $_data_zh['portfolio_st'],

            'seo_title'      => $_data_zh['seo_title'],
            'seo_description'=> $_data_zh['seo_description'],
        );


        $back_url = admin_url($this->_control."/items");
        $insert = $this->history_model->duplicateItem($params, $params_vi, $params_en, $params_zh);

        if ($insert) {
            $image_blog = $this->history_model->getImageByBlogId($id, " AND image_type = '$this->_post_type' ");
    ...
    */
    function items($start=0){
        ...
        */
        ...
        $totalItems  = $this->history_model->countItems($cond_total);
        ...
        $items = $this->history_model->getItems($cond_items, $per_page, $start);
        $this->_data['items'] = $items;
    ...
    function deletemulti() {
        ...
        foreach ($checkAll as $id) {
            $this->history_model->id = $id;
            // $data = $this->history_model->getItemById();
            $this->history_model->updateBlogByFields( array('avail' => 0) );
        }
    ...
    function removemulti() {
        ...
        foreach ($checkAll as $id) {
            $this->history_model->id = $id;
            $data = $this->history_model->getItemById(" AND c.image_type = '$this->_post_type' ");
            @unlink($this->_path_upload.$data['path_image']);
            @unlink($this->_path_upload.$data['path_image_thumb']);
            $this->history_model->deleteItem( array('blog_id'=>$id) );
        }
    ...
    function restore() {
        ...
        if($blog_id && $blog_id > 0){
            $this->history_model->id = $blog_id;
            $this->history_model->updateBlogByFields(array('avail' => 1));
        }
    ...
    function update_status() {
        ...
        if($blog_id != '') {
            $this->history_model->id = $blog_id;
            echo $this->history_model->updateBlogByFields(array('avail'=>$avail));
            return;
        }
    ...
    function updateVerify() {
        ...
        if($blog_id != '') {
            $this->history_model->id = $blog_id;
            echo $this->history_model->updateBlogByFields( array('admin_verify' => $display_home) );
            return;
        }
    ...
    function show_home() {
        ...
        if($blog_id != '') {
            $this->history_model->id = $blog_id;
            $this->history_model->current_lang = $lang;
            echo $this->history_model->updateBlogTranslateByFields( array('home_status' => $display_home) );
            return;
        }
    ...
    function do_slim_upload($blog_id) {
        ...
        if($new_img){
            ...
            if($old_img != '' && $old_file_name != $filename) {
                ...
                $imageOldExist = $this->history_model->getImageByBlogId($blog_id, $cond=" AND image_type = '".$this->_post_type."' ");
                if($imageOldExist != '') {
                    ...
                    $this->history_model->updateImage($params, $image_id);
                } else {
                    $this->history_model->insertImage($params);
                }
                $flag_change_img = 1;
            } else { // insert
                $this->history_model->insertImage($params);
