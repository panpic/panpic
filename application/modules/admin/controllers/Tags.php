...
class Tags extends MY_Controller{
    ...
    public function __construct(){
        ...
        $this->_post_type = POST_TYPE_TAGS;
        // this->_blog_parent_category = PARENT_CAT_TESTIMONIAL;
        // $this->_data['blog_parent_category'] = $this->_blog_parent_category;
        ...
        $this->tags_model->current_lang = $this->current_lang;
        $this->tags_model->page_lang = $this->page_lang;
        $this->tags_model->default_lang = $this->default_lang;
    ...
    function index(){
        ...
        $this->_data['breadcrumb'] = $this->lable['tags'];
        $this->_data['alert'] = '';
        ...
        if (!empty($id) && $option == 'edit') {
            $this->tags_model->id = $id;
            $data = $this->tags_model->getItemsMultiLangById();
            $first = $data[0];
    ...
    function add() {
        ...
        $params = $this->tags_model->builParamsInsert($_data, $this->current_lang);

        if ( !empty($id) && $option == OPTION_EDIT ) {

            $this->tags_model->id = $id;
            $update = $this->tags_model->updateItem($params);
            ...
            $insert = $this->tags_model->insertItem($params);
    ...
    function duplicate() {
        ...
        $_data_vi = $this->tags_model->getItemByCond($cond_vi, $cond_img);
        $slug_vi = $this->tags_model->get_slug_exist($_data_vi['slug']."-".DUPLICATED);
        ...
        $_data_en = $this->tags_model->getItemByCond($cond_en, $cond_img);
        $slug_en = $this->tags_model->get_slug_exist($_data_en['slug']."-".DUPLICATED);
        $params_en = array(
            'lang'          => $_data_en['lang'],
            'slug'          => $slug_en,
        ...
        $_data_zh = $this->tags_model->getItemByCond($cond_zh, $cond_img);
        $slug_zh = $this->tags_model->get_slug_exist($_data_zh['slug']."-".DUPLICATED);
        $params_zh = array(
            'lang'          => $_data_zh['lang'],
            'slug'          => $slug_zh,
        ...
        $insert = $this->tags_model->duplicateItem($params, $params_vi, $params_en, $params_zh);

        if ($insert) {
            $image_blog = $this->tags_model->getImageByBlogId($id, " AND image_type = '$this->_post_type' ");
    ...
    function items($start=0){
        ...
        if($keyword){
            $cond .= " WHERE a.title LIKE '%$keyword%' ";
            $more_url .= "q=$keyword";
            $this->_data['search'] = $keyword;
        }

        $this->_data['alert'] = '';
        $this->_data['breadcrumb'] = $this->lable['mn_technology'];

        $cond_items = " AND i.image_type = '$this->_post_type' $cond ";
        $cond_total = " $cond ";

        $totalItems  = $this->tags_model->countItems($cond_total);
        ...
        $items = $this->tags_model->getItems($cond_items, $per_page, $start);
        $this->_data['items'] = $items;
        // pre($items);
    ...
    function deletemulti() {
        ...
        foreach ($checkAll as $id) {
            $this->tags_model->id = $id;
            // $data = $this->tags_model->getItemById();
            $this->tags_model->updateBlogByFields( array('avail' => 0) );
        }
    ...
    function removemulti() {
        ...
        foreach ($checkAll as $id) {
            $this->tags_model->id = $id;
            $data = $this->tags_model->getItemById(" AND c.image_type = '$this->_post_type' ");
            @unlink($this->_path_upload.$data['path_image']);
            @unlink($this->_path_upload.$data['path_image_thumb']);
            $this->tags_model->deleteItem( array('blog_id'=>$id) );
        }
    ...
    function restore() {
        ...
        if($blog_id && $blog_id > 0){
            $this->tags_model->id = $blog_id;
            $this->tags_model->updateBlogByFields(array('avail' => 1));
        }
    ...
    function update_status() {
        ...
        if($blog_id != '') {
            $this->tags_model->id = $blog_id;
            echo $this->tags_model->updateBlogByFields(array('avail'=>$avail));
            return;
        }
    ...
    function updateVerify() {
        ...
        if($blog_id != '') {
            $this->tags_model->id = $blog_id;
            echo $this->tags_model->updateBlogByFields( array('admin_verify' => $display_home) );
            return;
        }
    ...
    function show_home() {
        ...
        if($blog_id != '') {
            $this->tags_model->id = $blog_id;
            $this->tags_model->current_lang = $lang;
            echo $this->tags_model->updateBlogByFields( array('home_status' => $display_home) );
            die();
        }
    ...
    function do_slim_upload($blog_id) {
        ...
        if($new_img){
            ...
            if($old_img != '' && $old_file_name != $filename) {
                ...
                $imageOldExist = $this->tags_model->getImageByBlogId($blog_id, $cond=" AND image_type = '".$this->_post_type."' ");
                if($imageOldExist != '') {
                    ...
                    $this->tags_model->updateImage($params, $image_id);
                } else {
                    $this->tags_model->insertImage($params);
                }
                $flag_change_img = 1;
            } else { // insert
                $this->tags_model->insertImage($params);
