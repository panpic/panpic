<?php
/**
* Model Backend Product category
...
class Productcat_model extends MY_Model
{

	private $_table = 'pp_product_category';
	private $_tableDesc = 'pp_product_category_desc';
    
    private $_fields = ' dscr.cat_name, dscr.cat_slug, dscr.cat_icon, dscr.seo_title, dscr.seo_description ';

	public $_lang = 'vi';
    public $_parent = 0;
    public $_data;
    public $_id = 'product_cat_id';
    public $_orderArr;
    public $product_cat_id;
	public $page_lang;
	public $default_lang;
    
    function getKeyString() { return " product_cat_id='$this->product_cat_id' AND lang='$this->_lang' "; }
    
    
    function keyUpdateMain($product_cat_id) {
        return array("product_cat_id" => $product_cat_id);
    }


    function keyUpdate($product_cat_id) {
        return  array('product_cat_id' => $product_cat_id, 'lang' => $this->_lang);
    }


	function get_slug_exist($slug, $cat_id, $parent_id='') {
		$cond = ($cat_id != '') ? " AND product_cat_id <> $cat_id " : '';

		$sql = "SELECT cat_slug FROM $this->_tableDesc WHERE cat_slug = '$slug' $cond";
		$exist_slug = $this->db->query($sql)->row()->cat_slug;

		if($exist_slug) {
			$parent_slug = '';
			if($parent_id != '') {
				$sql = "SELECT cat_slug FROM $this->_tableDesc WHERE product_cat_id = $parent_id";
				$parent_slug = $this->db->query($sql)->row()->cat_slug;
			}
			$exist_slug .= ($parent_slug != '') ? '-'.$parent_slug : $this->_lang."-1";
    ...
    function updateItem($params, $dscr, $product_cat_id){
        
        $this->db->trans_begin(); 
        
        if($params != '') {
            $this->db->update( $this->_table, $params, $this->keyUpdateMain($product_cat_id) );
        }

        if($dscr != '') {
            $this->db->update( $this->_tableDesc, $dscr, $this->keyUpdate($product_cat_id) );
        }
    ...
    */	
    function updateNode($data, $id = null, $newParentId = 0){
        ...
        if($id != null && $id != 0){
			...
			$params = array('posts_no' => $data['posts_no']);
	...
	*/
	function moveUp($id){
		...
		if(!empty($nodeBrother)){			
			$options = array('position'=>'before','brother_id'=>$nodeBrother['product_cat_id']);
			$this->moveNode($id, $parentInfo['product_cat_id'], $options);
		}
	...
	*/
	public function moveDown($id){
            ...
            if(!empty($nodeBrother)){			
                $options = array('position'=>'after','brother_id'=>$nodeBrother['product_cat_id']);
                $this->moveNode($id,$parentInfo['product_cat_id'],$options);
            }
	}
	...
	function orderTree($data, $orderArr){
		...
		foreach ($orderGroup as $key => $val){
			...
			foreach ($val as $key2 => $val2){
				...
				$tmpVal[$info['product_cat_id']] = $val2;
			}
			$orderGroup[$key] = $tmpVal;
		}
	...
	*/
	protected function getNodeByLeft($left){
		...
		$sql = 'SELECT c.*, dscr.cat_name, dscr.seo_title, dscr.seo_keyword, dscr.seo_description FROM '.$this->_table.' AS c JOINE '.$this->_tableDesc.' AS dscr ON c.product_cat_id=dscr.product_cat_id AND c.lft = '.$left;
			...
			$orderArr = array();
		 	if(count($data)>0){
		 		foreach ($data as $key => $val){
		 			$orderArr[$val['product_cat_id']] = array();
	...
	*/
	protected function moveBefore($brother_id){
		...
		$newParent 	= $infoParentNode['product_cat_id'];
		$newLeft 	= $infoBrotherNode['lft'];
		$newRight 	= $infoBrotherNode['lft'] + $widthMoveNode - 1;
		$slqUpdateParent = 'UPDATE ' .$this->_table.'  
						  SET parents = '.$newParent.', lft = '.$newLeft.', rgt = '.$newRight.' WHERE product_cat_id = '.$this->_id;
	...
	*/
	protected function movetAfter($brother_id){
		...
		$slqUpdateParent = 'UPDATE '.$this->_table.'  SET parents = '.$newParent.', lft = '.$newLeft.', rgt = '.$newRight.' WHERE product_cat_id = '.$this->_id;	
		$this->db->query($slqUpdateParent);
	...
	*/
	protected function insertRight(){
		...
		$this->db->trans_begin();
			...
			$c['posts_no']	= $this->_data['posts_no'];
			...
			foreach ($this->page_lang as $k=>$vl) {
				...
				$cat_slug = $this->get_slug_exist($this->_data['cat_slug'], '', $this->_parent);

				// If english not input
				if ($k != $this->default_lang) {
					$d['cat_slug'] = "$cat_slug-$k";
				} else {
					$d['cat_slug'] = $cat_slug;
				}
	...
	*/
	protected function insertLeft(){
            ...
            $this->db->trans_begin();
                ...
                $c['product_cat_id']   = $this->_data['product_cat_id'];		
                $c['parents']       = $this->_parent;
                $c['lft']           = $parentLeft + 1;
                $c['rgt']           = $parentLeft + 2;
                $c['level']         = $parentInfo['level'] + 1;

                $dscr['product_cat_id']= $this->_data['product_cat_id'];
                $dscr['cat_name']   = $this->_data['cat_name'];
				$dscr['cat_slug']	= $this->_data['cat_slug'];
	...
	*/
	protected function removeBranch(){
		...
		$sql = 'SELECT product_cat_id FROM '.$this->_table.' WHERE lft BETWEEN '.$lftNodeRemove.' AND '.$rgtNodeRemove;
		...
		$slqUpdateRight = 'UPDATE '.$this->_table.' SET rgt = (rgt - '.$widthNodeRemove.') WHERE rgt > '.$rgtNodeRemove;
                ...
                if(sizeof($arr) > 0) {
                    foreach ($arr as $vl) {
                        $this->db->delete($this->_tableDesc, array('product_cat_id' => $vl['product_cat_id']) );
                    }	
                }
	...
	*/
	protected function removeOne(){
		...
		$sql = 'SELECT product_cat_id FROM '.$this->_table.' WHERE parents = '.$nodeInfo['product_cat_id'].' ORDER BY lft ASC';		
		...
		if(count($childIds) >0){
			foreach ($childIds as $key => $val){
				...
				$options = array('position'=>'after','brother_id'=>$nodeInfo['product_cat_id']);
				$this->moveNode($id, $parent, $options);
			}
			$this->removeNode($nodeInfo['product_cat_id']);
		}
	...
	*/
	function getNodeInfo($id){
		...
		$sql = "SELECT c.*, $this->_fields FROM $this->_table AS c 
			JOIN $this->_tableDesc AS dscr ON c.product_cat_id = dscr.product_cat_id AND c.product_cat_id = $id AND dscr.lang ='".$this->_lang."' ";
	...
	*/
	function listItem($parents = 0, $items = 'all', $exclude_id = null, $level = 0, $num='', $offset=''){
		...
		$sqlItems = 'SELECT node.*, '.$this->_fields.' FROM '.$this->_table.' AS node 
				JOIN '.$this->_tableDesc.' AS dscr ON node.product_cat_id = dscr.product_cat_id AND dscr.lang ="'.$this->_lang.'" ';
		...
		if($exclude_id != null && $exclude_id >0){
			$sqlExclude = '	SELECT lft, rgt FROM '.$this->_table.' WHERE product_cat_id = '.$exclude_id;			
	...
	*/
	function countItem($level = 0){
		...
		$sql = 'SELECT COUNT(product_cat_id) AS total FROM '.$this->_table;	
	...
	function countItemByCond($cond, $level=0) {
		...
		$sql = "SELECT COUNT(c.product_cat_id) AS total FROM $this->_table AS c 
				JOIN $this->_tableDesc AS dscr ON c.product_cat_id=dscr.product_cat_id AND dscr.lang ='$this->_lang' $cond";
	...
	function items($level = 0, $num =0, $offset =0)
	{
		...
		$sql = 'SELECT c.*, '.$this->_fields.' FROM '.$this->_table.' AS c 
				JOIN '.$this->_tableDesc.' AS dscr ON c.product_cat_id=dscr.product_cat_id AND dscr.lang ="'.$this->_lang.'" ';
		
		if($level != 0){ $sql .= ' AND c.level <= '.$level; }
		
		echo $sql .= ' ORDER BY c.lft ASC '.$limit;
	...
	function itemsByCond($cond = 0, $num =0, $offset =0, $level=0)
	{
		...
		$sql = "SELECT c.*, $this->_fields FROM $this->_table AS c 
				...
				ON c.product_cat_id=dscr.product_cat_id AND dscr.lang ='$this->_lang' $cond ORDER BY c.lft ASC $limit";
	...
	function parentNode($cond, $num =0, $offset =0, $order=' ORDER BY dscr.cat_name ASC ') {
		...
		$sql = 'SELECT c.*, '.$this->_fields.' FROM '.$this->_table.' AS c JOIN '.$this->_tableDesc.' AS dscr ON c.product_cat_id=dscr.product_cat_id ';
	...
	function getNodeByParentId($parent_id, $cond='', $order=' ORDER BY c.lft ASC ') {
		...
		$sql = "SELECT c.*, $this->_fields  
				...
				JOIN $this->_tableDesc AS dscr ON c.product_cat_id=dscr.product_cat_id  
				JOIN (SELECT p.* FROM $this->_table AS p WHERE p.product_cat_id = $parent_id) AS parent 
				ON c.lft >= parent.lft AND c.rgt <= parent.rgt AND dscr.lang = '$this->_lang' $cond $order";
	...
	function parentNodeHome($cond, $num =0, $offset =0, $order=' ORDER BY dscr.cat_name ASC ')
	{
		...
		$sql = 'SELECT '.$this->my_fields.' FROM '.$this->_table.' AS c 
			JOIN '.$this->_tableDesc.' AS dscr ON c.product_cat_id = dscr.product_cat_id '.$cond.$order.$limit;
	...
	*/
	function breadcrumbs($id, $level_stop = null){
		...
		$sqls = "SELECT parent.*, $this->_fields FROM $this->_table AS node JOIN $this->_table AS parent
				ON (node.lft BETWEEN parent.lft AND parent.rgt) AND node.product_cat_id = $id AND parent.parents <> 0 ";
		
		if(isset($level_stop)){ $sqls .= " AND parent.level > $level_stop "; }
		
		$sqls .= " JOIN $this->_tableDesc AS dscr ON parent.product_cat_id=dscr.product_cat_id ";
	...
	function catStrIN($product_cat_id)
	{
            if( empty($product_cat_id) ) return ;

			$sql = " SELECT mc.product_cat_id, mcdscr.cat_name FROM $this->_table as mc RIGHT JOIN ( SELECT c.*, dscr.cat_name FROM $this->_table AS c JOIN $this->_tableDesc AS dscr ON c.product_cat_id = dscr.product_cat_id AND c.product_cat_id=$product_cat_id ) AS test ON (mc.lft >=test.lft AND mc.rgt <= test.rgt) JOIN $this->_tableDesc AS mcdscr ON mc.product_cat_id = mcdscr.product_cat_id ORDER BY mc.product_cat_id ASC ";

            $arr = $this->db->query($sql)->result('array'); //$this->db->fetchAll($sqls);				
            $catStrIN = $product_cat_id;

            if(sizeof($arr) > 0) {
                foreach ($arr as $vl) {	$catStrIN .= ','.$vl['product_cat_id']; }
            }
	 ...
	 * @return array
	 */
	function itemsTwoLevel($product_cat_id) {
		if(empty($product_cat_id)) return ;
		
		$sql = "SELECT mc.product_cat_id, mc.parents, mc.level, mc.lft, mc.rgt, mcdscr.cat_name FROM ".$this->_table." as mc
			RIGHT JOIN
				( SELECT c.*, dscr.cat_name FROM ".$this->_table." AS c 
					JOIN ".$this->_tableDesc." AS dscr ON c.product_cat_id = dscr.product_cat_id AND c.product_cat_id =$product_cat_id 
				) AS test ON (mc.rgt < test.rgt) AND mc.lft > test.lft
			JOIN ".$this->_tableDesc." AS mcdscr ON mc.product_cat_id = mcdscr.product_cat_id 
	...
	function allItems($cond='AND node.product_cat_id != 1') {
		$sql = "SELECT node.product_cat_id, node.parents, node.level, node.lft, node.rgt, dscr.cat_name FROM es_hs_code_vn AS node JOIN es_hs_code_vn_desc AS dscr ON node.product_cat_id=dscr.product_cat_id AND node.mcAvail=1 $cond ORDER BY node.lft ASC";
		return $this->db->query($sql)->result('array'); //$this->db->fetchAll($sql);		
	}
	 ...
	 * @return array
	 */
	function twoLevelPrev($product_cat_id) {
		if(empty($product_cat_id)) return ;
		
		$sql = "SELECT mc.product_cat_id, mc.parents, mc.level, mcdscr.cat_name FROM {$this->_table} as mc 
				RIGHT JOIN ( 
					SELECT c.*, dscr.cat_name FROM {$this->_table} AS c 
					JOIN {$this->_tableDesc} AS dscr ON c.product_cat_id = dscr.product_cat_id AND c.product_cat_id=$product_cat_id 
					) AS test ON (test.product_cat_id =  mc.product_cat_id OR test.parents=mc.product_cat_id)
					JOIN {$this->_tableDesc} AS mcdscr ON mc.product_cat_id = mcdscr.product_cat_id 
	...
	function parentNodeCustomField($num =0, $offset =0) {		
		$limit = ($num > 0) ? " LIMIT $offset,$num " : '';	
		$sql = "SELECT c.product_cat_id, d.cat_name FROM $this->_table AS c JOIN $this->_tableDesc AS d ON c.product_cat_id=d.product_cat_id AND (c.parents=1 OR c.level=1) AND c.mcAvail=1 ORDER BY d.cat_name ASC $limit";
		return $this->db->query($sql)->result('array'); //$this->db->fetchAll($sql);	
	}
