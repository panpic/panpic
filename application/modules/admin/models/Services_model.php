...
class Services_model extends MY_Model
{
    ...
    function getItemById($cond_image=" AND c.image_type = 'B' "){
        ...
        $sql = "SELECT a.category_id, a.portfolio_year, a.post_type, 
                a.date_add, a.avail, b.*, c.image_id, c.path_image, c.path_image_thumb
        ...
        $this->db->trans_begin();
            ...
            foreach ($this->page_lang as $k=>$vl) {
                ...
                if($k != $this->default_lang && $params_translate[$k]['title'] == '') {
                    ...
                    $slug = $this->get_slug_exist($slug, $this->id, " AND lang != '$k' ");

                    $data = array(
                        'blog_id'   => $this->id,
                        'lang'      => $k,
                        'slug'      => $slug,
                        'title'     => $params_translate[$k]['title']
                    );
                } else {
    ...
    function getItems($cond='', $num='', $offset=''){
        ...
        $sql = "SELECT a.category_id, a.post_type, a.date_add, a.avail, a.admin_verify, b.*, d.cat_name, d.cat_slug, i.path_image, i.path_image_thumb 
    ...
    function builParamsInsert($_data, $lang) {
        ...
        return array(
            ...
            'portfolio_utility' => $_data[$lang]['portfolio_utility'],
            'portfolio_clients' => $_data[$lang]['portfolio_clients'],
            'portfolio_skills'  => $_data[$lang]['portfolio_skills'],
            'seo_title'     => trim(strip_tags($_data[$lang]['seo_title'])),
            'seo_description'=> trim(strip_tags($_data[$lang]['seo_description']))
        );

        /*
            'price'         => $_data[$lang]['price'],
            'portfolio_ntc' => $_data[$lang]['portfolio_ntc'],
            'portfolio_ntdul' => $_data[$lang]['portfolio_ntdul'],
            'portfolio_qm' => $_data[$lang]['portfolio_qm'],
            'portfolio_dt' => $_data[$lang]['portfolio_dt'],
            'portfolio_st' => $_data[$lang]['portfolio_st'],
            'content_2'     => $_data[$lang]['content_2'],
            */
    }
