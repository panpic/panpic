...
class Tourcat_model extends MY_Model
{

	private $_table = 'tour_category';
	private $_tableDesc = 'tour_category_desc';
    
    private $_fields = ' dscr.cat_name, dscr.seo_title, dscr.seo_description ';
    private $_lang = 'vi';
    
    public $_parent = 0;	
    public $_data;
    public $_id = 'tour_cat_id';
    public $_orderArr;
    public $tour_cat_id;
    
    function getKeyString() { return " tour_cat_id='$this->tour_cat_id' AND lang='$this->_lang' "; }
    
    
    function keyUpdateMain($tour_cat_id) {
        return array("tour_cat_id" => $tour_cat_id);
    }

    function keyUpdate($tour_cat_id) {
        return  array('tour_cat_id' => $tour_cat_id, 'lang' => $this->_lang);
    }
        
    function updateItem($params, $dscr, $tour_cat_id){
        
        $this->db->trans_begin(); 
        
        if($params != '') {
            $this->db->update( $this->_table, $params, $this->keyUpdateMain($tour_cat_id) );
        }

        if($dscr != '') {
            $this->db->update( $this->_tableDesc, $dscr, $this->keyUpdate($tour_cat_id) );
        }
    ...
    */	
    function updateNode($data, $id = null, $newParentId = 0){
        ...
        if($id != null && $id != 0){
            ...
            $update = $this->updateItem('', $dscr, $id);
        }
	...
	*/
	function moveUp($id){
		...
		if(!empty($nodeBrother)){			
			$options = array('position'=>'before','brother_id'=>$nodeBrother['tour_cat_id']);
			$this->moveNode($id, $parentInfo['tour_cat_id'], $options);
		}
	...
	*/
	public function moveDown($id){
            ...
            if(!empty($nodeBrother)){			
                $options = array('position'=>'after','brother_id'=>$nodeBrother['tour_cat_id']);
                $this->moveNode($id,$parentInfo['tour_cat_id'],$options);
            }
	}
	...
	*/
	protected function getNodeByLeft($left){
		...
		$sql = 'SELECT c.*, dscr.cat_name, dscr.seo_title, dscr.seo_keyword, dscr.seo_description FROM '.$this->_table.' AS c JOINE '.$this->_tableDesc.' AS dscr ON c.tour_cat_id=dscr.tour_cat_id AND c.lft = '.$left;
			...
			$orderArr = array();
		 	if(count($data)>0){
		 		foreach ($data as $key => $val){
		 			$orderArr[$val['tour_cat_id']] = array();
	...
	*/
	protected function moveBefore($brother_id){
		...
		$newParent 	= $infoParentNode['tour_cat_id'];
		$newLeft 	= $infoBrotherNode['lft'];
		$newRight 	= $infoBrotherNode['lft'] + $widthMoveNode - 1;
		$slqUpdateParent = 'UPDATE ' .$this->_table.'  
						  SET parents = '.$newParent.', lft = '.$newLeft.', rgt = '.$newRight.' WHERE tour_cat_id = '.$this->_id;
	...
	*/
	protected function movetAfter($brother_id){
		...
		$slqUpdateParent = 'UPDATE '.$this->_table.'  SET parents = '.$newParent.', lft = '.$newLeft.', rgt = '.$newRight.' WHERE tour_cat_id = '.$this->_id;	
		$this->db->query($slqUpdateParent);
	...
	*/
	protected function insertLeft(){
            ...
            $this->db->trans_begin();
                ...
                $c['tour_cat_id']   = $this->_data['tour_cat_id'];		
                $c['parents']       = $this->_parent;
                $c['lft']           = $parentLeft + 1;
                $c['rgt']           = $parentLeft + 2;
                $c['level']         = $parentInfo['level'] + 1;

                $dscr['tour_cat_id']= $this->_data['tour_cat_id'];
                $dscr['cat_name']   = $this->_data['cat_name'];
				$dscr['cat_slug']	= $this->_data['cat_slug'];
	...
	*/
	protected function removeBranch(){
		...
		$sql = 'SELECT tour_cat_id FROM '.$this->_table.' WHERE lft BETWEEN '.$lftNodeRemove.' AND '.$rgtNodeRemove;
		...
		$slqUpdateRight = 'UPDATE '.$this->_table.' SET rgt = (rgt - '.$widthNodeRemove.') WHERE rgt > '.$rgtNodeRemove;
                ...
                if(sizeof($arr) > 0) {
                    foreach ($arr as $vl) {
                        $this->db->delete($this->_tableDesc, array('tour_cat_id' => $vl['tour_cat_id']) );
                    }	
                }
	...
	*/
	protected function removeOne(){
		...
		$sql = 'SELECT tour_cat_id FROM '.$this->_table.' WHERE parents = '.$nodeInfo['tour_cat_id'].' ORDER BY lft ASC';		
		...
		if(count($childIds) >0){
			foreach ($childIds as $key => $val){
				...
				$options = array('position'=>'after','brother_id'=>$nodeInfo['tour_cat_id']);
				$this->moveNode($id, $parent, $options);
			}
			$this->removeNode($nodeInfo['tour_cat_id']);
		}
	...
	*/
	function getNodeInfo($id){
		...
		$sql = "SELECT c.*, $this->_fields FROM $this->_table AS c 
			JOIN $this->_tableDesc AS dscr ON c.tour_cat_id = dscr.tour_cat_id AND c.tour_cat_id = $id AND dscr.lang ='".$this->_lang."' ";
	...
	*/
	function listItem($parents = 0, $items = 'all', $exclude_id = null, $level = 0, $num='', $offset=''){
		...
		$sqlItems = 'SELECT node.*, '.$this->_fields.' FROM '.$this->_table.' AS node 
				JOIN '.$this->_tableDesc.' AS dscr ON node.tour_cat_id = dscr.tour_cat_id AND dscr.lang ="'.$this->_lang.'" ';
		...
		if($exclude_id != null && $exclude_id >0){
			$sqlExclude = '	SELECT lft, rgt FROM '.$this->_table.' WHERE tour_cat_id = '.$exclude_id;			
	...
	*/
	function countItem($level = 0){
		...
		$sql = 'SELECT COUNT(tour_cat_id) AS total FROM '.$this->_table;	
	...
	function countItemByCond($cond, $level=0) {
		...
		$sql = "SELECT COUNT(c.tour_cat_id) AS total FROM $this->_table AS c 
				JOIN $this->_tableDesc AS dscr ON c.tour_cat_id=dscr.tour_cat_id AND dscr.lang ='$this->_lang' $cond";
	...
	function items($level = 0, $num =0, $offset =0)
	{
		...
		$sql = 'SELECT c.*, '.$this->_fields.' FROM '.$this->_table.' AS c 
				JOIN '.$this->_tableDesc.' AS dscr ON c.tour_cat_id=dscr.tour_cat_id AND dscr.lang ="'.$this->_lang.'" ';
	...
	function itemsByCond($cond = 0, $num =0, $offset =0, $level=0)
	{
		...
		$sql = "SELECT c.*, $this->_fields FROM $this->_table AS c 
				...
				ON c.tour_cat_id=dscr.tour_cat_id AND dscr.lang ='$this->_lang' $cond ORDER BY c.lft ASC $limit";
	...
	function parentNode($cond, $num =0, $offset =0, $order=' ORDER BY dscr.cat_name ASC ') {
		...
		$sql = 'SELECT c.*, '.$this->_fields.' FROM '.$this->_table.' AS c JOIN '.$this->_tableDesc.' AS dscr ON c.tour_cat_id=dscr.tour_cat_id ';
	...
	function getNodeByParentId($parent_id, $cond='', $order=' ORDER BY c.lft ASC ') {
		...
		$sql = "SELECT c.*, $this->_fields  
				...
				JOIN $this->_tableDesc AS dscr ON c.tour_cat_id=dscr.tour_cat_id  
				JOIN (SELECT p.* FROM $this->_table AS p WHERE p.tour_cat_id = $parent_id) AS parent 
				ON c.lft >= parent.lft AND c.rgt <= parent.rgt $order";
	...
	function parentNodeHome($cond, $num =0, $offset =0, $order=' ORDER BY dscr.cat_name ASC ')
	{
		...
		$sql = 'SELECT '.$this->my_fields.' FROM '.$this->_table.' AS c 
			JOIN '.$this->_tableDesc.' AS dscr ON c.tour_cat_id = dscr.tour_cat_id '.$cond.$order.$limit;
	...
	*/
	function breadcrumbs($id, $level_stop = null){
		...
		$sqls = "SELECT parent.*, $this->_fields FROM $this->_table AS node JOIN $this->_table AS parent
				ON (node.lft BETWEEN parent.lft AND parent.rgt) AND node.tour_cat_id = $id AND parent.parents <> 0 ";
		
		if(isset($level_stop)){ $sqls .= " AND parent.level > $level_stop "; }
		
		$sqls .= " JOIN $this->_tableDesc AS dscr ON parent.tour_cat_id=dscr.tour_cat_id ";
	...
	function catStrIN($tour_cat_id)
	{
            if( empty($tour_cat_id) ) return ;

			$sql = " SELECT mc.tour_cat_id, mcdscr.cat_name FROM $this->_table as mc RIGHT JOIN ( SELECT c.*, dscr.cat_name FROM $this->_table AS c JOIN $this->_tableDesc AS dscr ON c.tour_cat_id = dscr.tour_cat_id AND c.tour_cat_id=$tour_cat_id ) AS test ON (mc.lft >=test.lft AND mc.rgt <= test.rgt) JOIN $this->_tableDesc AS mcdscr ON mc.tour_cat_id = mcdscr.tour_cat_id ORDER BY mc.tour_cat_id ASC ";

            $arr = $this->db->query($sql)->result('array'); //$this->db->fetchAll($sqls);				
            $catStrIN = $tour_cat_id;

            if(sizeof($arr) > 0) {
                foreach ($arr as $vl) {	$catStrIN .= ','.$vl['tour_cat_id']; }
            }
	 ...
	 *
	 * @param int $tour_cat_id
	 * @return array
	 */
	function itemsTwoLevel($tour_cat_id) {
		if(empty($tour_cat_id)) return ;
		
		$sql = "SELECT mc.tour_cat_id, mc.parents, mc.level, mc.lft, mc.rgt, mcdscr.cat_name FROM ".$this->_table." as mc
			RIGHT JOIN
				( SELECT c.*, dscr.cat_name FROM ".$this->_table." AS c 
					JOIN ".$this->_tableDesc." AS dscr ON c.tour_cat_id = dscr.tour_cat_id AND c.tour_cat_id =$tour_cat_id 
				) AS test ON (mc.rgt < test.rgt) AND mc.lft > test.lft
			JOIN ".$this->_tableDesc." AS mcdscr ON mc.tour_cat_id = mcdscr.tour_cat_id 
	...
	function allItems($cond='AND node.tour_cat_id != 1') {
		$sql = "SELECT node.tour_cat_id, node.parents, node.level, node.lft, node.rgt, dscr.cat_name FROM es_hs_code_vn AS node JOIN es_hs_code_vn_desc AS dscr ON node.tour_cat_id=dscr.tour_cat_id AND node.mcAvail=1 $cond ORDER BY node.lft ASC";
		return $this->db->query($sql)->result('array'); //$this->db->fetchAll($sql);		
	}
	 ...
	 * @return array
	 */
	function twoLevelPrev($tour_cat_id) {
		if(empty($tour_cat_id)) return ;
		
		$sql = "SELECT mc.tour_cat_id, mc.parents, mc.level, mcdscr.cat_name FROM {$this->_table} as mc 
				RIGHT JOIN ( 
					SELECT c.*, dscr.cat_name FROM {$this->_table} AS c 
					JOIN {$this->_tableDesc} AS dscr ON c.tour_cat_id = dscr.tour_cat_id AND c.tour_cat_id=$tour_cat_id 
					) AS test ON (test.tour_cat_id =  mc.tour_cat_id OR test.parents=mc.tour_cat_id)
					JOIN {$this->_tableDesc} AS mcdscr ON mc.tour_cat_id = mcdscr.tour_cat_id 
	...
	function parentNodeCustomField($num =0, $offset =0) {		
		$limit = ($num > 0) ? " LIMIT $offset,$num " : '';	
		$sql = "SELECT c.tour_cat_id, d.cat_name FROM $this->_table AS c JOIN $this->_tableDesc AS d ON c.tour_cat_id=d.tour_cat_id AND (c.parents=1 OR c.level=1) AND c.mcAvail=1 ORDER BY d.cat_name ASC $limit";
		return $this->db->query($sql)->result('array'); //$this->db->fetchAll($sql);	
	}
