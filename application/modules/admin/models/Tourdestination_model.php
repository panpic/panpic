...
/**
* Model Backend Tour destination (arrived)
...
class Tourdestination_model extends MY_Model {


    var $_table = 'cat_destination';
    var $_tableDesc = 'cat_destination_desc';

    private $_fields = ' dscr.tour_home, dscr.cat_name, dscr.slug, dscr.cat_name_unicode, dscr.cat_name_lable, dscr.cat_icon, dscr.home, dscr.starting_latitude, dscr.starting_longtitude, dscr.seo_title, dscr.seo_description ';
    private $_lang = 'vi';

    public $_parent = 0;
    public $_data;
    public $_id = 'tour_destination_id';
    public $_orderArr;
    public $tour_destination_id;

    function getKeyString() { return " tour_destination_id = $this->tour_destination_id AND lang='$this->_lang' "; }


    function keyUpdateMain($tour_destination_id) {
        return array('tour_destination_id' => $tour_destination_id);
    }


    function keyUpdate($tour_destination_id) {
        return array('tour_destination_id' => $tour_destination_id, 'lang' => $this->_lang);
    }



    function updateItem($params, $dscr, $tour_destination_id){
        $this->db->trans_begin(); 

            if($params) {
                $this->db->update( $this->_table, $params, $this->keyUpdateMain($tour_destination_id) );
            }

            if($dscr) {
                $this->db->update( $this->_tableDesc, $dscr, $this->keyUpdate($tour_destination_id) );
            }
    ...
    */
    function updateNode($data, $id = null, $newParentId = 0){
        ...
        if($id != null && $id != 0){
			...
			$dscr['slug']           	= $data['slug'];
            $dscr['cat_name_unicode']   = $data['cat_name_unicode'];
            $dscr['cat_name_lable']     = $data['cat_name_lable'];
            $dscr['starting_latitude']  = $data['starting_latitude'];
            $dscr['starting_longtitude']= $data['starting_longtitude'];
	...
	*/
	function moveUp($id){
		...
		if(!empty($nodeBrother)){
			$options = array('position'=>'before','brother_id'=>$nodeBrother['tour_destination_id']);
			$this->moveNode($id, $parentInfo['tour_destination_id'], $options);
		}
	...
	*/
	public function moveDown($id){
		...
		if(!empty($nodeBrother)){
			$options = array('position'=>'after','brother_id'=>$nodeBrother['tour_destination_id']);
			$this->moveNode($id,$parentInfo['tour_destination_id'],$options);
		}
	}
	...
	function orderTree($data, $orderArr){
		...
		foreach ($orderGroup as $key => $val){
			...
			foreach ($val as $key2 => $val2){
				...
				$tmpVal[$info['tour_destination_id']] = $val2;
			}
	...
	*/
	protected function getNodeByLeft($left){
		...
		$sql = 'SELECT c.*, dscr.cat_name, dscr.seo_title, dscr.seo_description FROM '.$this->_table.' AS c JOINE '.$this->_tableDesc.' AS dscr ON c.tour_destination_id=dscr.tour_destination_id AND c.lft = '.$left;
			...
			$orderArr = array();
		 	if(count($data)>0){
		 		...
		 		foreach ($data as $key => $val){
		 			$orderArr[$val['tour_destination_id']] = array();
	...
	*/
	protected function moveBefore($brother_id){
		...
		$newParent 	= $infoParentNode['tour_destination_id'];
		$newLeft 	= $infoBrotherNode['lft'];
		$newRight 	= $infoBrotherNode['lft'] + $widthMoveNode - 1;
		$slqUpdateParent = 'UPDATE ' .$this->_table.'  
						  SET parents = '.$newParent.', lft = '.$newLeft.', rgt = '.$newRight.' WHERE tour_destination_id = '.$this->_id;
	...
	*/
	protected function movetAfter($brother_id){
		...
		$slqUpdateParent = 'UPDATE '.$this->_table.'  SET parents = '.$newParent.', lft = '.$newLeft.', rgt = '.$newRight.' WHERE tour_destination_id = '.$this->_id;	
	...
	*/
	protected function insertRight(){
		...
		$this->db->trans_begin();
			...
			// $c['destinationtour']	= $this->_data['destinationtour'];
			// $c['showsearch']	= $this->_data['showsearch'];
			...
			$dscr['slug']           	= $this->_data['slug'];
			$dscr['cat_name_unicode']   = $this->_data['cat_name_unicode'];
			$dscr['cat_name_lable']     = $this->_data['cat_name_lable'];
			...
			$dscr['home'] = $this->_data['home'];
			$dscr['link_gmap'] = $this->_data['link_gmap'];
			...
			$dscr['tour_destination_id'] = $last_id;
	...
	*/
	protected function insertLeft(){
            ...
            $this->db->trans_begin();
                ...
                $c['tour_destination_id']   = $this->_data['tour_destination_id'];		
                $c['destinationtour']	= $this->_data['destinationtour'];
                $c['showsearch']	= $this->_data['showsearch'];
                $c['parents']       = $this->_parent;
                ...
                $dscr['tour_destination_id']= $this->_data['tour_destination_id'];
                $dscr['cat_name']           = $this->_data['cat_name'];
                $dscr['cat_name_unicode']   = $this->_data['cat_name_unicode'];
                $dscr['cat_name_lable']     = $this->_data['cat_name_lable'];
                $dscr['starting_latitude']  = $this->_data['starting_latitude'];
                ...
                $dscr['tour_home'] 		= $this->_data['tour_home'];
				$dscr['link_gmap'] 		= $this->_data['link_gmap'];
				$dscr['seo_title']		= $this->_data['seo_title'];
                $dscr['seo_description']= $this->_data['seo_description'];
	...
	*/
	protected function removeBranch(){
		...
		$sql = 'SELECT tour_destination_id FROM '.$this->_table.' WHERE lft BETWEEN '.$lftNodeRemove.' AND '.$rgtNodeRemove;
		...
		$slqUpdateRight = 'UPDATE '.$this->_table.' SET rgt = (rgt - '.$widthNodeRemove.') WHERE rgt > '.$rgtNodeRemove;
                ...
                if(sizeof($arr) > 0) {
                    foreach ($arr as $vl) {
                        $this->db->delete($this->_tableDesc, array('tour_destination_id' => $vl['tour_destination_id']) );
                    }
                }
	...
	*/
	protected function removeOne(){
		...
		$sql = 'SELECT tour_destination_id FROM '.$this->_table.' WHERE parents = '.$nodeInfo['tour_destination_id'].' ORDER BY lft ASC';
		...
		if(count($childIds) >0){
			foreach ($childIds as $key => $val){
					...
					$options = array('position'=>'after','brother_id'=>$nodeInfo['tour_destination_id']);
					$this->moveNode($id, $parent, $options);
			}

			$this->removeNode($nodeInfo['tour_destination_id']);
		}
	}
	...
	*/
	function getNodeInfo($id){
		...
		$sql = "SELECT c.*, $this->_fields FROM $this->_table AS c 
				JOIN $this->_tableDesc AS dscr ON c.tour_destination_id = dscr.tour_destination_id AND c.tour_destination_id = $id AND dscr.lang ='$this->_lang'";
	...
	*/
	function listItem($parents = 0, $items = 'all', $exclude_id = null, $level = 0, $num='', $offset=''){
		...
		$sqlItems = 'SELECT node.*, '.$this->_fields.' FROM '.$this->_table.' AS node 
				JOIN '.$this->_tableDesc.' AS dscr ON node.tour_destination_id = dscr.tour_destination_id AND dscr.lang ="'.$this->_lang.'" ';
		...
		if($exclude_id != null && $exclude_id >0){
			$sqlExclude = '	SELECT lft, rgt FROM '.$this->_table.' WHERE tour_destination_id = '.$exclude_id;
	...
	*/
	function countItem($level = 0){
		...
		$sql = 'SELECT COUNT(tour_destination_id) AS total FROM '.$this->_table;	
	...
	function countItemByCond($cond, $level=0) {
		...
		$sql = "SELECT COUNT(c.tour_destination_id) AS total FROM $this->_table AS c 
				JOIN $this->_tableDesc AS dscr ON c.tour_destination_id=dscr.tour_destination_id AND dscr.lang ='$this->_lang' $cond";
	...
	function items($level = 0, $num =0, $offset =0)
	{
		...
		$sql = 'SELECT c.*, '.$this->_fields.' FROM '.$this->_table.' AS c 
				JOIN '.$this->_tableDesc.' AS dscr ON c.tour_destination_id=dscr.tour_destination_id AND dscr.lang ="'.$this->_lang.'" ';
	...
	function itemsByCond($cond = 0, $num =0, $offset =0, $level=0) {
        ...
        $sql = "SELECT c.*, $this->_fields FROM $this->_table AS c 
				...
				ON c.tour_destination_id=dscr.tour_destination_id AND dscr.lang ='$this->_lang' $cond ORDER BY c.lft ASC $limit";
	...
	function parentNode($cond, $num =0, $offset =0, $order=' ORDER BY dscr.cat_name ASC ') {
		...
		$sql = 'SELECT c.*, '.$this->_fields.' FROM '.$this->_table.' AS c JOIN '.$this->_tableDesc.' AS dscr ON c.tour_destination_id=dscr.tour_destination_id ';
	...
	function parentNodeHome($cond, $num =0, $offset =0, $order=' ORDER BY dscr.cat_name ASC ') {
		...
		$sql = 'SELECT '.$this->my_fields.' FROM '.$this->_table.' AS c 
			JOIN '.$this->_tableDesc.' AS dscr ON c.tour_destination_id = dscr.tour_destination_id '.$cond.$order.$limit;
	...
	*/
	function breadcrumbs($id, $level_stop = null){
		...
		$sql = "SELECT parent.*, $this->_fields FROM $this->_table AS node JOIN $this->_table AS parent
				ON (node.lft BETWEEN parent.lft AND parent.rgt) AND node.tour_destination_id = $id AND parent.parents <> 0 ";

		if(isset($level_stop)){ $sql .= " AND parent.level > $level_stop "; }

		$sql .= " JOIN $this->_tableDesc AS dscr ON parent.tour_destination_id=dscr.tour_destination_id ";
	...
	function catStrIN($tour_destination_id)

	{

            if( empty($tour_destination_id) ) return ;



            $sqls = " SELECT mc.tour_destination_id, mcdscr.cat_name FROM $this->_table as mc RIGHT JOIN ( SELECT c.*, dscr.cat_name FROM $this->_table AS c JOIN $this->_tableDesc AS dscr ON c.tour_destination_id = dscr.tour_destination_id AND c.tour_destination_id=$tour_destination_id ) AS test ON (mc.lft >=test.lft AND mc.rgt <= test.rgt) JOIN $this->_tableDesc AS mcdscr ON mc.tour_destination_id = mcdscr.tour_destination_id ORDER BY mc.tour_destination_id ASC ";



            $arr = $this->db->query($sql)->result('array'); //$this->db->fetchAll($sqls);				

            $catStrIN = $tour_destination_id;



            if(sizeof($arr) > 0) {

                foreach ($arr as $vl) {	$catStrIN .= ','.$vl['tour_destination_id']; }
	 ...
	 * @param int $tour_destination_id

	 * @return array

	 */

	function itemsTwoLevel($tour_destination_id) {

		if(empty($tour_destination_id)) return ;

		

		$sql = "SELECT mc.tour_destination_id, mc.parents, mc.level, mc.lft, mc.rgt, mcdscr.cat_name FROM ".$this->_table." as mc

			RIGHT JOIN

				( SELECT c.*, dscr.cat_name FROM ".$this->_table." AS c 

					JOIN ".$this->_tableDesc." AS dscr ON c.tour_destination_id = dscr.tour_destination_id AND c.tour_destination_id =$tour_destination_id 

				) AS test ON (mc.rgt < test.rgt) AND mc.lft > test.lft

			JOIN ".$this->_tableDesc." AS mcdscr ON mc.tour_destination_id = mcdscr.tour_destination_id 
	...
	function allItems($cond='AND node.tour_destination_id != 1') {

		$sql = "SELECT node.tour_destination_id, node.parents, node.level, node.lft, node.rgt, dscr.cat_name FROM es_hs_code_vn AS node JOIN es_hs_code_vn_desc AS dscr ON node.tour_destination_id=dscr.tour_destination_id AND node.mcAvail=1 $cond ORDER BY node.lft ASC";
	...
	function twoLevelPrev($tour_destination_id) {

		if(empty($tour_destination_id)) return ;

		

		$sql = "SELECT mc.tour_destination_id, mc.parents, mc.level, mcdscr.cat_name FROM {$this->_table} as mc 
				...
				RIGHT JOIN ( 
					...
					) AS test ON (test.tour_destination_id =  mc.tour_destination_id OR test.parents=mc.tour_destination_id)
	...
	function parentNodeCustomField($num =0, $offset =0) {		
		...
		$sql = "SELECT c.tour_destination_id, d.cat_name FROM $this->_table AS c JOIN $this->_tableDesc AS d ON c.tour_destination_id=d.tour_destination_id AND (c.parents=1 OR c.level=1) AND c.mcAvail=1 ORDER BY d.cat_name ASC $limit";
