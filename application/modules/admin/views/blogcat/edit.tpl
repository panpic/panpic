...
<form class="form-horizontal margin-none" id="frm_data" name="frm_data" method="post" action="{$base_url_admin}/blogcat/processnested/" autocomplete="off" enctype="multipart/form-data">
<input type="hidden" name="option" id="option" value="{$option}" />
<input type="hidden" name="data[post_cat_id]" id="post_cat_id" value="{$data.post_cat_id}" />
...
<div class="row innerLR">
	<div class="col-md-12">
		...
		<div class="form-group">
			...
			<div class="col-md-10">
				<select data-placeholder="Danh mục.." class="col-md-6" name="data[parents]" id="select2_1">
						{$arrNested.cmb}
				</select>
		...
		<div class="form-group">
			<label class="col-md-2 control-label" for="lang_value">{$lable.category}(*)</label>
			<div class="col-md-7">
				<div class="input-group">
				<input type="text" name="data[cat_name]" id="mcCategory" value="{$data.cat_name|stripslashes}" class="form-control" placeholder="" />
		...
		<div class="form-group">
			<label class="col-md-2 control-label" for="lang_value">Slug(*)</label>
			<div class="col-md-7">
				<input type="text" name="data[cat_slug]" id="cat_slug" value="{$data.cat_slug|stripslashes}" class="form-control" placeholder="URL slug">
			</div>
		</div>
		<div class="form-group">
			<label class="col-md-2 control-label" for="cat_note">{$lable.cat_note}</label>
			<div class="col-md-7">
				<textarea name="data[cat_note]" id="cat_note" class="col-md-6 form-control">{$data.cat_note|stripslashes}</textarea>
			</div>
		</div>
	</div>
	</div><!-- // Column END -->
    <div class="col-md-12" style="padding-top:10px;">
		<div class="row">
			<div class="col-md-6">
				{include file="blogcat/slim_upload.tpl"}
			</div>
			<div class="col-md-6">
				<div class="row">
					<label class="col-md-4 control-label" for="cat_note">{$lable.pos}</label>
					<div class="col-md-2">
						<input type="text" name="data[posts_no]" id="posts_no" value="{$data.posts_no}" class="form-control">
					</div>
				</div>
				<div class="row">
					<label class="col-md-4 control-label" for="home_status">Hiện trang chủ</label>
					<div class="col-md-2">
						<input type="checkbox" name="data[home_status]" id="home_status" value="1" {if $data.home_status eq 1} checked{/if}>
					</div>
				</div>
			</div>
		</div>
