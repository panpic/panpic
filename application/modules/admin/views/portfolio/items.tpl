                    ...
                    <form method="get" action="" name="box_search">
						...
						<select name="cat" id="category" class="form-control">
							...
							{foreach from=$categories item=cat}
								{if $cat.parents gt 1}
									<option value="{$cat.post_cat_id}" {if $search.category_id eq $cat.post_cat_id} selected="selected"{/if}> {$cat.cat_name}</option>
								{/if}
							{/foreach}
						</select>
						</div>
                    ...
                    <table class="table list-items table-bordered table-condensed" id="dataTables-member">
                        <thead>
                            <tr>
                                ...
                                <th>{$lable.image}</th>
                                <th width="20%">{$lable.portfolio_name}</th>
                                {**<th>Logo các đơn vị</th>**}
                                <th>{$lable.gallery}</th>
                                <th>{$lable.date_add}</th>
                                <th>{$lable.last_modify}</th>
                                <th class="text-center">{$lable.hits}</th>
                                <th class="text-center">{$lable.lable_portfolio_typical_projects}</th>
                        ...
                        {foreach from=$items item=item}
                            <tr class="parent-status">
                                ...
                                {**
                                <td>
                                    {assign var=count_services_process value=$item.blog_id|count_services_process}
                                    <a href="{$base_url_admin}/{$control}/process?s={$item.blog_id}" class="btn btn-primary" title="Add images"><i class="fa fa-plus-circle"></i> ({$count_services_process})</a>
                                </td>**}
                                <td>
                                	{assign var=count_images value=$item.blog_id|count_gallery:$POST_TYPE_SERVICES_GALLERY}
                                	<a href="{$base_url_admin}/{$control}/gallery?album={$item.blog_id}" class="btn btn-primary" title="Add images"><i class="fa fa-plus-circle"></i> ({$count_images})</a>
                                </td>
