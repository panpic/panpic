                    ...
                    <table class="table list-items table-bordered table-condensed" id="dataTables-member">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center">{$lable.product_hot}</th>
                        ...
                        {foreach from=$items item=item}
                            <tr class="parent-status">
                                <td class="check-id">
                                  <input name="checkAll[]" value="{$item.product_id}" type="checkbox" />
                                </td>
                                ...
                                <td>
                                	<a href="{$base_url_admin}/{$control}/addimage?p={$item.product_id}" class="btn btn-primary"><i class="fa fa-plus-circle"></i> ({$item.product_id|count_gallery_product})</a>
                                </td>
                                <td>
                                {if $item.admin_verify eq $ADMIN_BLOG_VERIFY && $item.avail eq 1}
                                	{assign var=blog_url value=$item.slug|url_product_detail}
                                ...
                                <td class="text-center update-hot_status" id="hot_status-{$item.product_id}">
                                    <input type="checkbox" name="hot_status" class="hot_status" id="{$item.product_id}" value="{$item.hot_status}" {if $item.hot_status eq 1} checked="checked"{/if} />
                                </td>
                                <td class="text-center update-home_status" id="shows_home-{$item.product_id}">
                                    <input type="checkbox" name="shows_home-{$item.product_id}" class="show-home" id="{$item.product_id}" value="{$item.home_status}" rel="{$item.product_id}" data-lang="vi" {if $item.home_status eq 1} checked="checked"{/if} />
                                </td>
                                
                                <td class="text-center update-status" id="status-{$item.product_id}">
                                    <input type="checkbox" name="avail" class="avail" id="{$item.product_id}" value="{$item.avail}" {if $item.avail eq 1} checked="checked"{/if} />
                                </td>
                                <td style="text-align:center;">
                                	<a href="{$base_url_admin}/{$control}/duplicate/?id={$item.product_id}" class="btn btn-sm btn-success button_margin" title="Duplication item"><i class="fa fa-copy"></i></a> 
                                    <a href="{$base_url_admin}/{$control}/?id={$item.product_id}&option=edit" class="btn btn-sm btn-success button_margin" title="Update"><i class="fa fa-pencil"></i></a>
                                </td>
                            </tr>
                        {/foreach}
                            <tr>
...
<script src="{$base_tlp_admin}/js/product.js"></script>