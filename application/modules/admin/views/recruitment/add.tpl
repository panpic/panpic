                    ...
                    <div class="widget-body">
                        <div class="tab-content">
                            ...
                            <div class="tab-pane {if $k eq $default_lang}active{/if}" id="tab1-{$k}">
                                {include file="{$control}/form-post.tpl"}
                            </div>
                            {/foreach}
                        </div><!--.tab-content-->
                    </div>
...
$(document).ready (function(){ 
    ...
    jQuery(function(){
        jQuery('#career_effect').datetimepicker({
            format:'Y-m-d',
            onShow:function( ct ){
                this.setOptions({
                    maxDate:jQuery('#career_expire').val()?jQuery('#career_expire').val():false
                })
            },
            timepicker:false
        });
        jQuery('#career_expire').datetimepicker({
            format:'Y-m-d',
            onShow:function( ct ){
                this.setOptions({
                    minDate:jQuery('#career_effect').val()?jQuery('#career_effect').val():false
                })
            },
            timepicker:false
        });
