...
<div class="innerLR">
    ...
    <div class="widget widget-tabs widget-tabs-double widget-tabs-responsive">
        ...
        <div class="widget-head">
            {include file="{$control}/filter.tpl"}
        </div>
                    ...
                    <table class="table list-items table-bordered table-condensed" id="dataTables-member">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center">{$lable.tags}</th>
                        ...
                        {foreach from=$items item=item}
                            <tr class="parent-status">
                                <td class="check-id">
                                  <input name="checkAll[]" value="{$item.tag_id}" type="checkbox" />
                                </td>
                                ...
                                <td>{$item.content|stripslashes}</td>
                                <td class="text-center">{$item.hits}</td>
                                <td class="text-center">{$item.tags}</td>
                                <td style="text-align:center;">
                                {if $tab eq $BLOG_TAB_INACTIVE}
                                    <a href="{$base_url_admin}/{$control}/restore/?id={$item.tag_id}" class="btn btn-sm btn-success button_margin" title="{$lable.restore}"><i class="fa fa-undo"></i></a>
                                {else}
                                	<a href="{$base_url_admin}/{$control}/?id={$item.tag_id}&option=edit" class="btn btn-sm btn-success button_margin" title="Update"><i class="fa fa-pencil"></i></a>
                                {/if}
                                </td>
                            </tr>
                        {/foreach}
...
$(document).ready(function() { 
	...
	$('.show-home').on('click', function(e) {
        ...
        $.ajax({
            url: base_url_admin +'/testimonial/show_home',
            type: 'POST',
