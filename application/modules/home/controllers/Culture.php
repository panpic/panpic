 ...
 * @since 8 Jul 2020
 */
class Culture extends FRONT_Controller
{
    ...
    public function index($slug)
    {
        ...
        // Get news related
        $conNewsRelated = "post_type = '" . POST_TYPE_CULTURE . "' and category_id = '$news[category_id]' and blog_id != '$news[blog_id]'";
        ...
        $this->_data['seo'] = array(
            ...
            'seo_image' => $this->link_upload. '/' . $news['path_image']
        );
    ...
    public function culture()
    {
        ...
        $conNews = " post_type = '" . POST_TYPE_CULTURE . "' AND content IS NOT NULL AND content != ''";
        ...
        // Set breadcrumb
        $this->_data['breadcrumb'] = '<li class="is-active" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" aria-current="page"><span itemprop="name">' . $this->lable['company_culture'] . '</span><meta itemprop="position" content="3" /></li>';

        $this->parser->parse("culture/culture.tpl", $this->_data);
    }
    ...
    function category($slug)
    {
        ...
        // Get blog news by category slug, check news exist from database
        $conNews = " post_type = '" . POST_TYPE_CULTURE . "' AND category_id = '" . $category[0]['post_cat_id'] . "' AND content IS NOT NULL AND content != ''";
        $totalItems = $this->blog_model->counterBlog($conNews);
        ...
        // Set breadcrumb
        $this->_data['breadcrumb'] = '<li  itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . base_url($slugNews) . '"><span itemprop="name">' . $this->lable['company_culture'] . '</span></a><meta itemprop="position" content="2" /></li>
        <li class="is-active" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" aria-current="page"><span itemprop="name">' . $this->_data['category']['cat_name'] . '</span><meta itemprop="position" content="3" /></li>';

        $this->parser->parse("culture/category.tpl", $this->_data);
    }
    ...
    function page($slug = '')
    {
        ...
        <li class="is-active" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" aria-current="page"><span itemprop="name">' . stripslashes($page['page_title']) . '</span><meta itemprop="position" content="3" /></li>';
        }
        ...
        $this->parser->parse("page/fullscreen-no-title.tpl", $this->_data);
    }
    ...
    function award() {
        ...
        $slugNews = convertSlugByLang(CULTURE_SLUG);
        $PARENT_CAT_AWARD = PARENT_CAT_AWARD;

        $fields = 'post_cat_id, cat_name, cat_slug, parents, cat_note';
        $category = $this->main_model->menuCatBlogs(" WHERE parents = $PARENT_CAT_AWARD ", $fields);
        // $categories = $this->nestedpostcat_library->groupSubMenuByParent($all_category);
        // $menu_culture = $categories[PARENT_CAT_CULTURE]['sub'];
        // $sub = $this->category_ward['sub'];
        // pre($category);

        $field = "slug, blog_id, category_id, title, short, content, path_image, path_image_thumb, date_add, seo_title";
        $conNews = " post_type = '" . POST_TYPE_AWARD . "' AND content IS NOT NULL AND content != ''";
        $items = $this->blog_model->getBlog($conNews, '', '', $field);
        $temp = array();
        foreach ($items as $vl) {
            $temp[$vl['category_id']][] = $vl;
        }

        $this->_data['category'] = $category;
        $this->_data['items'] = $temp;

        // Set seo data
        $this->_data['seo'] = array(
            'seo_title' => $this->lable['seo_title_award'],
            'seo_description' => $this->lable['seo_desc_award'],
            'seo_image' => ''
        );

        // Set breadcrumb
        $this->_data['breadcrumb'] = '<li  itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . base_url($slugNews) . '"><span itemprop="name">' . $this->lable['company_culture'] . '</span></a><meta itemprop="position" content="2" /></li>
        <li class="is-active" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" aria-current="page"><span itemprop="name">' . $this->lable['award'] . '</span><meta itemprop="position" content="3" /></li>';


        $this->parser->parse( $this->control."/award.tpl", $this->_data);
    }
