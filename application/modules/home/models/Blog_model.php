...
class Blog_model extends MY_Model
{
    ...
    function getTagBySlug($slug, $fields='a.tag_id, a.title'){
        if(!$slug) return;

        $POST_TYPE_TAGS = POST_TYPE_TAGS;
        $sql = "SELECT $fields FROM pp_blog_tags AS a
                LEFT JOIN $this->_pp_images AS c ON a.tag_id = c.object_id AND c.image_type = '$POST_TYPE_TAGS'  
                WHERE a.slug = '$slug'";
        return $this->db->query($sql)->row_array();
    }
    ...
    function updateTagHit($id) {
        if(!$id) return;

        $sql = "UPDATE pp_blog_tags SET hits = hits+1 WHERE blog_id = $id";
