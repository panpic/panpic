            ...
            <div class="row">
                <div class="col-lg-8 order-lg-1">
                    <div class="heading mt-0">
                        <h1 class="heading-title lh-base h3">{$category.cat_name}</h1>
                    </div>
                    {if $totalItems == 0}
                    ...
                    {else}
                        <div class="row gx-2 gx-sm-3">
                            {foreach from=$news key=$newsKey item=newsItem}
                                {assign var=link value=$newsItem.slug|url_safety_detail:$newsItem.blog_id}
                                {if $newsKey == 0}
                                    <div class="col-12 mb-4">
                                        <h5 class="post-title post-title-lg">
                                            <a href="{$link}" title="{$newsItem.title|stripslashes}">{$newsItem.title|stripslashes}</a>
                                        </h5>
                                        ...
                                        <div class="post-thumb my-3">
                                            <a class="ani-opa05" href="{$link}" title="{$newsItem.title|stripslashes}">
