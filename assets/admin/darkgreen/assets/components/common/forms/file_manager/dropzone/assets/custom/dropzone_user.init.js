...
function checkMain() {
    ...
    $('.check_main').on('change keyup click', function () {
        ...
        var type = (term) ? 1 : 0;
		//alert( "file_main=" + file_main + "&type=" + type + "&product_id=" + product_id );
		
        if (file_main) {

            $.ajax({
                type: "POST",
                url: user_url +"myfile/mainfile/",
                async: false,
    ...
    if ($.fn.dropzone != 'undefined')
        $('.dropzone').dropzone({
			...
			maxfilesexceeded: function() {
				$('#max_upload_image').html(max_upload_product_image);
			},
