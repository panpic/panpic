...
$.fn.optionTree = function(tree, options) {
	...
	//sst value & add to input id
    var setValue = function(name, value) {
		$("input[name='" + cleanName(name) + "']").val(value).change();
		alert(value);
		//drop product list at step 2 2013-03-13
		if(value != '') {
			productsByCatId(value,'');
		}
    ...
    return this.each(function() {
        ...
        if (typeof tree === "object") { // many options exists for current nesting level
			...
			var flag = false; //2012-10-29
        }
		
		// control button map product 2013-03-13
		alert(flag);
		
		if(flag == false) {
			$('#'+options.btn_action).show();
		}else if(flag == true && $('#'+options.btn_action).is(":visible")) {
			$('#'+options.btn_action).hide();
			$('#cat_id').val(0);
		}
